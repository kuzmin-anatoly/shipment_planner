...
class LLMClient:
    ...
    async def chat(self, system_prompt: str, user_prompt: str) -> str:
        ...
        try:
            ...
            if settings.llm_offline_fallback:
                return (
                    "Local LLM is not available yet. The platform route worked, but LiteLLM/Ollama "
    ...
    async def _ollama_health(self) -> dict:
        try:
            ...
            return {"available": False, "provider": "ollama", "error": str(exc)}
    ...
    async def _ollama_chat(self, system_prompt: str, user_prompt: str) -> str:
        ...
        try:
            ...
            if settings.llm_offline_fallback:
                return (
                    "Ollama is not available yet, or the model is not downloaded. "
                    f"Expected model: {self.model}. Error: {exc}"
                )
            raise
